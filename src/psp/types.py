...
class Entry:
    ...
    # Old do-it-all validation function
    # (will be deleted when i finish the test code)
    ...
    #     # We don't really care about format at the moment
    #     # fmt = self.data['format']
    #     enc = self.data['encoding']
    #     if data is None:
    #         if path is None:
    #             raise ValueError("either 'raw' or 'source' should be set "
    #                              "in the 'data' attribute")
    #     if path is not None:
    #         self.__validate_source_path(path)
    #     if enc is None:
    #         raise ValueError("'encoding' is not set in the 'data' "
    #                          "attribute")
    #         if self.is_text():
    #             # TODO: validate encoding
    #             pass
    #     if type_ is None:
    #         raise ValueError("'type' is not set in the 'data' attribute")
    #         if not datatype.has_type(type_):
    #             raise ValueError(f'data type {type_!r} is not registered')
    ...
    # def __validate_source_path(self, source):
    #     if not isinstance(source, (str, os.PathLike)):
    #         raise TypeError(f'source path should be str or os.PathLike, '
    #                         f'not {source!r}')
    #     try:
    #         with io.open(source, 'rb') as fp:
    #             pass
    #     except OSError as exc:
