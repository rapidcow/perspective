...
``binary``.  The type of a big entry must be one of the registered *archive
formats*.  (Note that despite the name "archive *format*", it is not an entry
format, so you must use get_type() and set_type() and not their counterparts
for format.)
